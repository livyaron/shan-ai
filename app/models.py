...
class EvalGoldAnswer(Base):
    """Human-approved correct answer for an eval question. Immutable ground truth."""
    __tablename__ = "eval_gold_answers"
...
class ProjectDossier(Base):
    ...
    Aggregates the project's master-file row, project-linked memory notes, and
    recent snapshot history into one Groq-generated Hebrew brief. Regenerated by
    a drip cron (K per cycle) only when `source_hash` changes — never in bulk,
    to protect the Groq TPD budget. `is_dirty` marks it for the next drip.
    """
    ...
    is_dirty     = Column(Boolean, nullable=False, default=True, server_default="true", index=True)
