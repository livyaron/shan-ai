...
async def _ensure_eval_caches(session: AsyncSession | None = None) -> None:
    ...
    try:
        from app.models import QuerySynonym, PromptOverride, ProjectAlias, IntentOverride
...
def invalidate_eval_caches() -> None:
    """Force the next _ensure_eval_caches call to re-read from DB. Call after applying a patch."""
    global _EVAL_CACHE_TS
    _EVAL_CACHE_TS = 0.0
...
async def _apply_learned_synonyms(question: str, session: AsyncSession) -> str:
    ...
    try:
        ...
        # Eval-loop shadow synonyms: same matching rules, in-task only.
        for original, syns in _shadow_synonyms.get().items():
            original_forms = _question_word_forms(original)
            if original_forms & question_forms:
                question += " " + " ".join(syns)
                applied.append(f"shadow:{original}")
        if applied:
